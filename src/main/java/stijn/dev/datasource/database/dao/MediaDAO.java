...
public class MediaDAO {
    ...
    public List<String> getMediaTypesForPlatforms() {
        String triviaQuery = "MATCH (m:MediaType)-[:USES]-(platform:Platform) " +
                "RETURN DISTINCT m.Name ORDER BY m.Name";
        Result result = neo4JDatabaseHelper.runQuery(new Query(triviaQuery));
        ArrayList<String> media = new ArrayList<>();
        while(result.hasNext()) {
            Map<String, Object> row = result.next().asMap();
            media.add(String.valueOf(row.get("m.Name")));
        }
        return media;
    }

    public List<String> getPlatformMediaTypes(int id) {
        HashMap<String, Object> parameters = new HashMap<>();
        parameters.put("id", id);
        String triviaQuery = "MATCH (m:MediaType)-[:USES]-(platform:Platform) " +
                "WHERE ID(platform) = $id " +
